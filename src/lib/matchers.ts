...
import { PayvraDefaultError } from "../models/errors/payvradefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | PayvraDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | PayvraDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new PayvraDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new PayvraDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
